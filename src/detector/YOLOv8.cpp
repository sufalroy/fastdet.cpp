...
namespace fastdet::detector {
    ...
    void YOLOv8::draw(cv::Mat &image, const std::vector<Detection> &detections, const unsigned int scale) {
        ...
        const auto thickness = static_cast<int>(scale) + 1;
        const auto labelScale = LABEL_SCALE_FACTOR * scale;
        ...
        for (const auto &detection: detections) {
            ...
            const auto backgroundColorWithAlpha = BBOX_COLOR * BACKGROUND_ALPHA * 255;
    ...
    std::vector<std::vector<cv::cuda::GpuMat> > YOLOv8::preprocess(const cv::Mat &image) {
        ...
        if (rgbImage.rows != inputDim.d[1] || rgbImage.cols != inputDim.d[2]) {
            ...
            const auto scaledWidth = static_cast<int>(scaleRatio * rgbImage.cols);
            const auto scaledHeight = static_cast<int>(scaleRatio * rgbImage.rows);
    ...
    std::vector<Detection> YOLOv8::postprocess(const std::span<float> features) const {
        const auto &outputDims = mInferenceEngine->getOutputDims();
        ...
        const auto numClasses = static_cast<int>(mLabels.size());
        ...
        for (int i = 0; i < numAnchors; ++i) {
            ...
            const auto maxScore = *maxScoreIter;

            if (maxScore <= mProbabilityThreshold) {
                continue;
            }

            const auto centerX = bboxPtr[0];
            const auto centerY = bboxPtr[1];
            const auto width = bboxPtr[2];
            const auto height = bboxPtr[3];

            const auto x0 = std::clamp((centerX - 0.5f * width) * mImageMetrics.ratio,
                                       0.0f, mImageMetrics.width);
            const auto y0 = std::clamp((centerY - 0.5f * height) * mImageMetrics.ratio,
                                       0.0f, mImageMetrics.height);
            const auto x1 = std::clamp((centerX + 0.5f * width) * mImageMetrics.ratio,
                                       0.0f, mImageMetrics.width);
            const auto y1 = std::clamp((centerY + 0.5f * height) * mImageMetrics.ratio,
                                       0.0f, mImageMetrics.height);

            const auto classLabel = static_cast<int>(maxScoreIter - scoresPtr);
        ...
        const auto selectedIndices = nmsIndices
                                     ...
                                     | std::views::transform([&](const int idx) -> Detection {
                                         return {
                                             .label = labels[idx],
