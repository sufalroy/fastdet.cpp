    ...
    bool TensorRTEngine::build(const std::string &onnxPath, const Options &options) {
        ...
        for (int32_t i = 1; i < numInputs; ++i) {
            FASTDET_ASSERT_MSG(network->getInput(i)->getDimensions().d[0] == input0Batch,
                               "Model has multiple inputs with differing batch sizes");
        }
    ...
    bool TensorRTEngine::load(const std::string &enginePath, const std::array<float, 3> &subVals,
        ...
        for (int32_t i = 0; i < mEngine->getNbIOTensors(); ++i) {
            ...
            if (tensorType == nvinfer1::TensorIOMode::kINPUT) {
                FASTDET_ASSERT_MSG(tensorDataType == nvinfer1::DataType::kFLOAT, "Input tensor '{}' must be float32",
                                   tensorName);
                ...
                FASTDET_ASSERT_MSG(tensorDataType != nvinfer1::DataType::kINT8,
                                   "Output tensor '{}' has unsupported INT8 precision", tensorName);
                FASTDET_ASSERT_MSG(tensorDataType != nvinfer1::DataType::kFP8,
                                   "Output tensor '{}' has unsupported FP8 precision", tensorName);
                ...
                FASTDET_ASSERT_MSG(
                    cudaMallocAsync(&mBuffers[i], outputLength * mOptions.maxBatchSize * sizeof(float), stream) ==
                    cudaSuccess,
                    "GPU allocation failed for tensor: {}", tensorName);

                FASTDET_LOG_INFO("Output '{}': {} elements ({} bytes) allocated", tensorName, outputLength,
    ...
    bool TensorRTEngine::infer(const std::vector<std::vector<cv::cuda::GpuMat> > &input,
        ...
        for (size_t i = 1; i < input.size(); ++i) {
            if (input[i].size() != static_cast<size_t>(batchSize)) {
                FASTDET_LOG_ERROR("Inconsistent batch size: input[0] has {}, input[{}] has {}", batchSize, i,
                                  input[i].size());
                return false;
            }
        }
        ...
        for (size_t i = 0; i < numInputs; ++i) {
            ...
            FASTDET_LOG_INFO("Input '{}' configured: batch_size={}, shape={}x{}x{}", mIOTensorNames[i], batchSize,
        ...
        for (int32_t batch = 0; batch < batchSize; ++batch) {
            ...
            for (size_t outputIdx = 0; outputIdx < numOutputs; ++outputIdx) {
                ...
                const auto *srcPtr = static_cast<const char *>(mBuffers[outputBinding]) + (
